...
router.get('/plan-items', async (_req, res) => {
  try {
    const items = await prisma.planItem.findMany({ orderBy: [{ groupName: 'asc' }, { sortOrder: 'asc' }] });
    res.json(items);
...
router.post('/plan-items', async (req, res) => {
  try {
    ...
    const item = await prisma.planItem.create({
      data: { name: data.name, groupName: data.groupName || 'Other', paymentType: data.paymentType || 'Monthly', categoryId: data.categoryId ? +data.categoryId : null, keywordMatch: data.keywordMatch || null, sortOrder: data.sortOrder || 0, isActive: true },
    });
...
router.put('/plan-items/:id', async (req, res) => {
  try {
    ...
    const item = await prisma.planItem.update({
      ...
      data: { name: data.name, groupName: data.groupName, paymentType: data.paymentType, categoryId: data.categoryId ? +data.categoryId : null, keywordMatch: data.keywordMatch || null, sortOrder: data.sortOrder, isActive: data.isActive },
    });
    res.json(item);
  } catch (err) { res.status(500).json({ error: 'Failed' }); }
});
  ...
  try {
    ...
    for (const item of items) {
      ...
      for (const tx of transactions) {
        ...
        let matches = false;
        if (item.categoryId && tx.categoryId === item.categoryId) matches = true;
...
router.get('/annual', async (req, res) => {
  ...
  const transactions = await prisma.transaction.findMany({
    where: { date: { gte: new Date(`${y}-01-01`), lte: new Date(`${y}-12-31T23:59:59`) }, type: 'EXPENSE', categoryId: { not: null } },
...
router.post('/buckets', async (req, res) => {
  const d = req.body;
  res.status(201).json(await prisma.budgetBucket.create({ data: { name: d.name, targetPercent: d.targetPercent, categories: d.categories || [], color: d.color || '#6366f1', sortOrder: d.sortOrder || 0 } }));
});
router.put('/buckets/:id', async (req, res) => {
  const d = req.body;
  res.json(await prisma.budgetBucket.update({ where: { id: +req.params.id }, data: { name: d.name, targetPercent: d.targetPercent, categories: d.categories, color: d.color, sortOrder: d.sortOrder } }));
});
