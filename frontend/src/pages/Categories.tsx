...
export default function Categories() {
  ...
  const loadData = async () => {
    const [cats, sp] = await Promise.all([
      categoriesApi.all(),
      analyticsApi.categoriesSpending({
        startDate: period + '-01',
...
function SummaryView({ categories, spendMap, totalSpend, onEdit, onDelete }: any) {
  return (
              ...
              <tr key={cat.id} className="border-b border-gray-800/50 hover:bg-gray-900/30">
                <td className="p-3">
                  <div className="flex items-center gap-2">
                    ...
                    {cat.budgetGroup && <span className="text-xs text-gray-600 bg-gray-800 px-1.5 py-0.5 rounded">{cat.budgetGroup}</span>}
                  </div>
                </td>
                ...
                <td className="p-3">
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => onEdit(cat)} className="text-gray-600 hover:text-indigo-400"><Edit2 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => onDelete(cat.id)} className="text-gray-600 hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </td>
              </tr>
            )
...
function AnnualPlanView({ categories }: { categories: Category[] }) {
  ...
  const getFact = (catId: number, month: number) => {
    if (!annualData?.facts) return 0
    return annualData.facts[catId]?.[month] || 0
  }
  ...
  if (loading) return <div className="card text-center py-8 text-gray-500 animate-pulse">Loading annual plan...</div>
        ...
        <div className="flex gap-2">
          {[year - 1, year, year + 1].map(y => (
            <button key={y} onClick={() => setYear(y)}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium ${y === year ? 'bg-indigo-600 text-white' : 'btn-secondary'}`}
            >{y}</button>
          ))}
        </div>
      </div>
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="bg-gray-900">
              <th className="p-2 text-left text-gray-400 font-medium sticky left-0 bg-gray-900 z-10 border-b border-gray-800 min-w-32">Category</th>
              {MONTHS.map((m, i) => (
                <th key={i} colSpan={2} className="p-2 text-center text-gray-400 border-b border-l border-gray-800">{m}</th>
              ))}
            ...
            <tr className="bg-gray-900/50">
              ...
              {MONTHS.map((_, i) => (
                <>
                  <th key={`p${i}`} className="p-1.5 text-gray-500 border-b border-l border-gray-800 text-center">Plan</th>
                  <th key={`f${i}`} className="p-1.5 text-gray-500 border-b border-gray-800 text-center">Fact</th>
                </>
              ))}
              <th className="p-1.5 text-gray-500 border-b border-l border-gray-800 text-center">Plan</th>
              <th className="p-1.5 text-gray-500 border-b border-gray-800 text-center">Fact</th>
            </tr>
          </thead>
          <tbody>
            {categories.map(cat => {
              const totalPlan = MONTHS.reduce((sum, _, i) => sum + getPlan(cat.id, i + 1), 0)
              const totalFact = MONTHS.reduce((sum, _, i) => sum + getFact(cat.id, i + 1), 0)
              return (
                <tr key={cat.id} className="border-b border-gray-800/50 hover:bg-gray-900/30">
                      ...
                      <>
                        <td key={`p${mi}`} className="p-0 border-l border-gray-800 text-center">
                          {editing === key ? (
                            <input
                              type="number"
                              className="w-16 bg-indigo-900/50 text-indigo-300 text-xs p-1 text-center outline-none"
                              value={editValue}
                            ...
                            <button
                              className="w-full p-1.5 text-center text-gray-500 hover:text-indigo-400 hover:bg-indigo-900/20"
                  ...
                  <td className="p-1.5 text-center border-l border-gray-800 text-gray-400 font-medium">{totalPlan > 0 ? Math.round(totalPlan) : '—'}</td>
  ...
  return (
    <Modal title={cat ? 'Edit Category' : 'Add Category'} onClose={onClose} size="sm">
      <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
            {EMOJI_OPTIONS.map(e => (
              <button key={e} onClick={() => setIcon(e)}
                className={`text-xl p-1.5 rounded-lg ${icon === e ? 'bg-indigo-600' : 'hover:bg-gray-800'}`}
              >{e}</button>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="label">Budget Group (for rules)</label>
          <input className="input" value={budgetGroup} onChange={e => setBudgetGroup(e.target.value)} placeholder="needs, wants, savings..." />
        </div>
